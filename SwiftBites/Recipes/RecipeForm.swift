...
struct RecipeForm: View {
  ...
  private func ingredientPicker() -> some View {
    IngredientsView { selectedIngredient in
        print("Selected ingredient in RecipeForm: \(selectedIngredient.name)")
      let recipeIngredient = RecipeIngredientModel(ingredient: selectedIngredient, quantity: "")
      ingredients.append(recipeIngredient)
        print("Added ingredient: \(selectedIngredient.name)")
  ...
  func save() {
    //let category = storage.categories.first(where: { $0.id == categoryId })
      let cat = categoriesArray.first(where: { $0.id == categoryId } )
...
//        )
          //EDIT
          let newRecipeToAdd = RecipeModel(name: name, summary: summary, category: cat, serving: serving, time: time, ingredients: ingredients, instructions: instructions, imageData: imageData)
          context.insert(newRecipeToAdd)
      case .edit(let recipe):
